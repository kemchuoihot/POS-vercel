...
const Verify = () => {
    ...
    const handleChangeStatus = async (token) => {
        try {
            const response = await axios.get(`https://pos-vercel-server.vercel.app/verify/${token}`).catch((err) => {console.log(err);});
    ...
    const handlePasswordChange = async () => {
        try {
            const response = await axios.post('https://pos-vercel-server.vercel.app/changePass', { user,newPassword })
            .then
            (result =>{
